...
**File:** `/modules/conflictOverlay.js`

This module provides a set of utility functions for managing the UI of the sync conflict overlay. When `SyncManager` detects a conflict between local and cloud data, these functions are used to display the differences and handle user input.
...
These functions are imported into `popup.js` and used within wrapper functions to control the conflict overlay's visibility and content.
