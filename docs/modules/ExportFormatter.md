...
**File:** `/modules/exportFormatter.js`

The `ExportFormatter` class is used for converting a note's content and metadata into various formats for exporting, such as plain text, Markdown, and HTML.
...
The `ExportFormatter` is used in `popup.js` within the `exportNote` and `copyFormattedContent` functions to prepare content for downloading or copying to the clipboard.
