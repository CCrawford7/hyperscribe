...
**File:** `/modules/panelManager.js`

The `PanelManager` is responsible for managing the visibility and state of various UI panels, such as the theme selector, font controls, and emoji grid.
...
The `PanelManager` is instantiated in `popup.js` with a configuration of all manageable panels. Its methods are used to toggle panels and handle global key events for closing them.
